...
public :		
	...
	void init() 
	{
		...
		// VBO Vertex Buffer Object : stocke données qu'on va mettre sur la carte pour les dessiner
		// 3 éléments, 6 sommets
		// Permet de construire facilement des tableaux
		Vbo = new YVbo(3,6, YVbo::PACK_BY_VERTICE); //Deux Triangles, avec sommet, couleur et normale
		Vbo->setElementDescription(0, YVbo::Element(3)); //Sommet (Vec3)
		Vbo->setElementDescription(1, YVbo::Element(4)); //Couleur (RGBA)
		Vbo->setElementDescription(2, YVbo::Element(3)); //Normale (Vec3)
		Vbo->createVboCpu();

		//3 sommets
		Vbo->setElementValue(0, 0, 0, 0, 0); // = Pour 0, l'élément 0 est 0, 0, 0
		Vbo->setElementValue(0, 1, 1, 0, 0); // = Pour 0, l'élément 1 est 1, 0, 0
		Vbo->setElementValue(0, 2, 0, 1, 0); // etc...
		...
		Vbo->createVboGpu(); // malloc sur le GPU
		Vbo->deleteVboCpu();

		//On cree le VBO random : 3 éléments, 1 million de sommets puis valeur random attribuée
